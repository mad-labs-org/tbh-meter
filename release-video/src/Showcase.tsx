...
import { ScrollText, Zap, Coins, Star } from "lucide-react";
import {
  FPS, C, GRAD, SANS, MONO, clamp, ease, fmt, clock, CountUp,
...
export const Intro: React.FC = () => {
  ...
  const kicker = version ? `WHAT'S NEW · ${version}` : "THE COMMUNITY METER";
  return (
    <AbsoluteFill style={{ justifyContent: "center", alignItems: "center" }}>
      ...
      <div style={{ marginTop: 22, fontSize: 142, fontWeight: 800, letterSpacing: -3, color: C.text, lineHeight: 1 }}>
        <span style={word(6)}>TBH&nbsp;</span>
        <span style={{ ...word(12), background: GRAD, WebkitBackgroundClip: "text", backgroundClip: "text", color: "transparent", filter: `drop-shadow(0 0 ${glow}px rgba(116,133,255,0.5))` }}>Meter</span>
      </div>
      <div style={{ marginTop: 16, height: 4, width: lineW, background: GRAD, borderRadius: 2 }} />
      <div style={{ marginTop: 28, fontSize: 31, color: C.z400, fontWeight: 500, opacity: sub, transform: `translateY(${interpolate(sub, [0, 1], [16, 0])}px)` }}>a live DPS, gold &amp; EXP meter for Task Bar Hero</div>
    </AbsoluteFill>
  );
};
...
export const SceneOutro: React.FC<{ dur: number }> = ({ dur }) => {
  ...
  return (
    <AbsoluteFill style={{ justifyContent: "center", alignItems: "center", opacity: out }}>
      ...
      <div style={{ marginTop: 20, fontSize: 112, fontWeight: 800, letterSpacing: -2, transform: `scale(${interpolate(s, [0, 1], [0.94, 1])})`, background: GRAD, WebkitBackgroundClip: "text", backgroundClip: "text", color: "transparent", filter: `drop-shadow(0 0 ${glow}px rgba(116,133,255,0.5))` }}>TBH Meter</div>
      <div style={{ marginTop: 18, fontFamily: MONO, fontSize: 26, fontWeight: 600, color: C.z400, opacity: s }}>github.com/mad-labs-org/tbh-meter</div>
