...
export interface RunHero { heroKey: number; class: string; classId: number | null; level: number; exp: number; items: RunItem[]; skills: RunSkill[]; skillLevels?: Record<string, number>; stats: Record<string, number>; expStart?: number; expEnd?: number; levelup?: boolean; xpGained?: number; deaths?: number; revives?: number; killedBy?: number[]; /** Formation slot 0/1/2 — the hero's position in the live party (StageManager.HeroList). Absent on older runs. */ slot?: number; }
...
export interface LiveSnapshot { runNumber: number | null; stage: string; mode: string; stageKey: number | null; mobs: number; totalMobs: number | null; elapsedSec: number; damage: number; dps: number; goldGain: number | null; xpGain: number | null; party: number[] | null; /** Live chest-drop counts by EMonsterLogType index: [common(0), stageBoss(1), actBoss(2)]. */ drops: number[] | null; /** Live chest-open aggregates: [total(0), regular(1), blue(2)]. */ boxOpens: number[] | null; /** Live FINAL_STATS per deployed hero — `{heroKey: {statId: value}}` (numeric keys). null when an older reader doesn't emit it (the resistance tooltip then hides). */ partyStats: Record<number, Record<number, number>> | null; /** Live per-hero leveling — `{heroKey: {level, exp (within-level), gain (run xp)}}` (numeric keys). null when an older reader doesn't emit it (the time-to-level chip then hides). */ partyProgress: Record<number, { level: number; exp: number; gain: number }> | null; approx: boolean; }
